...
export default function SettingsPage() {
  ...
  const { session, refreshProfile, isLoading: isAuthLoading } = useAuth();
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setPageError(error instanceof Error ? error.message : 'Unable to load settings right now.');
    } finally {
  ...
  const savePreferences = async () => {
    ...
    try {
      ...
      emitAppToast({
        ...
        description: 'Your dashboard preferences are now up to date.',
        tone: 'success',
      });
    } catch (error) {
      emitAppToast({
  ...
  const saveNotifications = async () => {
    ...
    try {
      ...
      emitAppToast({
        ...
        description: 'Your notification choices have been saved.',
        tone: 'success',
      });
    } catch (error) {
      emitAppToast({
  ...
  const savePrivacy = async () => {
    ...
    try {
      ...
      emitAppToast({
        ...
        description: 'Your sharing and visibility preferences have been saved.',
        tone: 'success',
      });
    } catch (error) {
      emitAppToast({
  ...
  if (!session && !isAuthLoading) {
    return (
      <DashboardLayout
        title="Settings"
        subtitle="Manage your student profile, notification preferences, privacy options, and account security from one place."
      >
        <div className="dashboard-panel rounded-[1.8rem] p-6">
          ...
          <p className="mt-3 text-sm leading-7 text-[var(--dashboard-muted)]">
            Your session has ended. Please sign in again to continue managing your settings.
          </p>
          <button
            type="button"
    ...
    >
      {anyDirty ? (
        <div className="dashboard-soft-tile rounded-[1.4rem] border border-[var(--dashboard-accent-foreground)] px-4 py-3 text-sm text-[var(--dashboard-text)]">
          You have unsaved changes in this page.
        </div>
      ) : null}
          ...
          <section className="grid gap-4 xl:grid-cols-[1.08fr_0.92fr]">
            ...
            <div className="grid gap-4">
              ...
              <div className="dashboard-panel rounded-[1.8rem] p-5">
                <p className="text-sm font-semibold text-[var(--dashboard-text)]">Account guidance</p>
                <div className="mt-4 space-y-3">
                  <div className="dashboard-soft-tile rounded-[1.2rem] px-4 py-4 text-sm leading-6 text-[var(--dashboard-muted)]">
                    Keep your school, course, and contact details current so your certificates and readiness profile stay accurate.
                  </div>
                  <div className="dashboard-soft-tile rounded-[1.2rem] px-4 py-4 text-sm leading-6 text-[var(--dashboard-muted)]">
                    Save each section explicitly so you can review changes before they affect your assessment workflow.
                  </div>
                </div>
              </div>
            </div>
